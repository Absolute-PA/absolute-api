...
# Read device identity from .env.device (local-only, safe to read before git ops).
...
# TARGET_VERSION_API in .env.device is written by install-version.sh when a specific version is installed.
# Remove that key (or run install-version.sh with "auto-update") to return to hub-manifest mode.
HUB_URL="https://hub.absolutepa.com.au"
TARGET_VERSION=""

if [ -f "$DEVICE_ENV" ]; then
    _pinned=$(grep "^TARGET_VERSION_API=" "$DEVICE_ENV" | head -1 | cut -d'=' -f2-)
    if [ -n "$_pinned" ]; then
        TARGET_VERSION="$_pinned"
        echo "Resolved api version from .env.device pin: $TARGET_VERSION"
    fi
fi
...
# Report startup to hub (non-fatal — hub may be unreachable on some networks).
if [ -n "$PI_NAME" ]; then
